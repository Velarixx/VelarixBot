...
// VelarixBot server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "VelarixBot: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
const TOOLS = [
  {
    name: "approve",
    description: "Ask the VelarixBot user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "velarixbot-permissions", version: "1" },
      },
    });
  }
    ...
    const text = isQuestion
      ...
      : JSON.stringify(
          answer.behavior === "allow"
            ...
            : { behavior: "deny", message: answer.message || "Denied from VelarixBot" },
        );
