    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-app p-4">
      <div className="flex w-full max-w-[460px] flex-col rounded-2xl border border-hairline/40 bg-panel p-8">
        ...
        {step === 1 && <div className="flex flex-col">
          ...
          <p className="mt-1 text-[13.5px] text-ink-secondary">OpenMausBot uses each CLI directly with its existing login or OAuth session. Choose a model explicitly for every bot; it never silently fails over.</p>
          ...
          <button onClick={() => isElectron ? setStep(2) : finish()} className="mt-5 w-full rounded-lg bg-accent py-2.5 text-[15px] font-medium text-white">{isElectron ? "Continue" : "Start using OpenMausBot"}</button>
        ...
        {step === 2 && <div className="flex flex-col">
          ...
          <button onClick={finish} className="mt-5 w-full rounded-lg bg-accent py-2.5 text-[15px] font-medium text-white">Start using OpenMausBot</button>
          <button onClick={finish} className="mt-3 text-[12px] text-ink-secondary hover:text-ink">Skip for now</button>
        </div>}
      </div>
    </div>
