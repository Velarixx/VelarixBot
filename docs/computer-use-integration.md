# Computer use & browser use in VelarixBot

Decision doc, 2026-08-12. How bots in VelarixBot get local computer use and
...
│     one TCC prompt, named VelarixBot          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is VelarixBot's, users see ONE
   prompt named VelarixBot, and the bundled driver inherits it. Spawned from
